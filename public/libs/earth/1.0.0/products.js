...
var products = function() {
    ...
    function loadMockAQGrid(product, cancel) {
        var header = {
            lo1: 0,
            la1: 90,
            dx: 1.0,
            ...
            forecastTime: 0,
            centerName: "AirSense AQI Engine"
        };
        
        var data = [];
        var numPoints = header.nx * header.ny;
        var baseVal = product.baseValue;
        var maxVal = product.maxValue;
        
        for (var j = 0; j < header.ny; j++) {
            var lat = header.la1 - j * header.dy;
            for (var i = 0; i < header.nx; i++) {
                var lon = header.lo1 + i * header.dx;
                var displayLon = lon > 180 ? lon - 360 : lon;
                
                // Distance to Bandar Lampung (-5.45, 105.27)
                var dLat = lat - (-5.45);
                var dLon = displayLon - 105.27;
                if (dLon > 180) dLon -= 360;
                if (dLon < -180) dLon += 360;
                var distBL = Math.sqrt(dLat*dLat + dLon*dLon);
                
                // Distance to Jakarta (-6.2, 106.8)
                var dLatJkt = lat - (-6.2);
                var dLonJkt = displayLon - 106.8;
                if (dLonJkt > 180) dLonJkt -= 360;
                if (dLonJkt < -180) dLonJkt += 360;
                var distJkt = Math.sqrt(dLatJkt*dLatJkt + dLonJkt*dLonJkt);
                
                // Distance to Beijing (39.9, 116.4)
                var dLatBjg = lat - 39.9;
                var dLonBjg = displayLon - 116.4;
                if (dLonBjg > 180) dLonBjg -= 360;
                if (dLonBjg < -180) dLonBjg += 360;
                var distBjg = Math.sqrt(dLatBjg*dLatBjg + dLonBjg*dLonBjg);
                
                // Global base background value
                var val = baseVal + Math.abs(Math.sin(lat * Math.PI / 180) * Math.cos(displayLon * Math.PI / 180)) * (maxVal - baseVal) * 0.15;
                
                // Bandar Lampung hotspot (radius ~ 4 degrees, high AQI)
                if (distBL < 6) {
                    var intensityBL = (6 - distBL) / 6;
                    val += intensityBL * (maxVal - baseVal) * 0.85;
                }
                
                // Jakarta hotspot (radius ~ 6 degrees, very high AQI)
                if (distJkt < 8) {
                    var intensityJkt = (8 - distJkt) / 8;
                    val += intensityJkt * (maxVal - baseVal) * 0.95;
                }
                
                // Beijing hotspot (radius ~ 10 degrees, extremely high AQI)
                if (distBjg < 10) {
                    var intensityBjg = (10 - distBjg) / 10;
                    val += intensityBjg * (maxVal - baseVal) * 1.15;
                }
        ...
        return when(cancel.requested ? null : _.extend(product, buildGrid(gridBuilder)));
    }
    ...
    var FACTORIES = {
        ...
        "wind": {
            matches: function(attr) {
                return attr.param === "wind" || attr.param === "air_quality";
            },
        ...
        "pm2_5": {
            matches: _.matches({param: "air_quality", overlayType: "pm2_5"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "pm2_5",
                    description: localize({
                        name: {en: "PM2.5", ja: "PM2.5"},
                        qualifier: {en: " @ Surface", ja: " @ 地上"}
                    }),
                    paths: [],
                    date: gfsDate(attr),
                    ...
                    maxValue: 150,
                    load: function(cancel) {
                        return loadMockAQGrid(this, cancel);
                    },
                    units: [
                        {label: "µg/m³", conversion: function(x) { return x; }, precision: 1}
                    ],
                    scale: {
                        bounds: [0, 150],
                        gradient: µ.segmentedColorScale([
                            [0,   [0, 228, 0]],     // Good (Green)
                            [12,  [255, 255, 0]],   // Moderate (Yellow)
                            [35,  [255, 126, 0]],   // Sensitive (Orange)
                            [55,  [255, 0, 0]],     // Unhealthy (Red)
                            [150, [143, 63, 151]]   // Very Unhealthy (Purple)
                        ])
                    }
                });
            }
        ...
        "pm10": {
            matches: _.matches({param: "air_quality", overlayType: "pm10"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "pm10",
                    description: localize({
                        name: {en: "PM10", ja: "PM10"},
                        qualifier: {en: " @ Surface", ja: " @ 地上"}
                    }),
                    paths: [],
                    date: gfsDate(attr),
        ...
        "no2": {
            matches: _.matches({param: "air_quality", overlayType: "no2"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "no2",
                    ...
                    units: [
                        {label: "ppb", conversion: function(x) { return x; }, precision: 1}
                    ],
                    scale: {
                        bounds: [0, 200],
                        gradient: µ.segmentedColorScale([
        ...
        "co": {
            matches: _.matches({param: "air_quality", overlayType: "co"}),
            create: function(attr) {
                return buildProduct({
                    field: "scalar",
                    type: "co",
                    ...
                    units: [
                        {label: "ppm", conversion: function(x) { return x; }, precision: 2}
                    ],
                    scale: {
                        bounds: [0, 15],
                        gradient: µ.segmentedColorScale([
