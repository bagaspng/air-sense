...
(function() {
    ...
    function getAQIStatus(value, type) {
        if (type === "pm2_5") {
            if (value <= 12) return { text: "Good", color: "#00E400" };
            if (value <= 35) return { text: "Moderate", color: "#FFFF00" };
            if (value <= 55) return { text: "Unhealthy for Sensitive Groups", color: "#FF7E00" };
        ...
        if (type === "pm10") {
            ...
            if (value <= 150) return { text: "Moderate", color: "#FFFF00" };
            if (value <= 250) return { text: "Unhealthy", color: "#FF0000" };
            return { text: "Very Unhealthy", color: "#8F3F97" };
        }
        if (type === "no2") {
            if (value <= 53) return { text: "Good", color: "#00E400" };
            if (value <= 100) return { text: "Moderate", color: "#FFFF00" };
            if (value <= 200) return { text: "Unhealthy", color: "#FF0000" };
            return { text: "Very Unhealthy", color: "#8F3F97" };
        }
        if (type === "co") {
            if (value <= 4.4) return { text: "Good", color: "#00E400" };
            if (value <= 9.4) return { text: "Moderate", color: "#FFFF00" };
            if (value <= 15.0) return { text: "Unhealthy", color: "#FF0000" };
            return { text: "Very Unhealthy", color: "#8F3F97" };
        }
        return null;
    }
    ...
    function showOverlayValueAtLocation(value, product) {
        ...
        // Show/hide AQI status row
        var aqiRow = d3.select("#location-aqi-status-row");
        var status = getAQIStatus(value, product.type);
        if (status) {
            aqiRow.classed("invisible", false);
            d3.select("#location-aqi-status")
                .text(status.text)
                .style("color", status.color)
                .style("font-weight", "bold");
        } else {
            aqiRow.classed("invisible", true);
        }
    }
    ...
    function clearLocationDetails(clearEverything) {
        ...
        d3.select("#location-aqi-status-row").classed("invisible", true);
        if (clearEverything) {
            activeLocation = {};
    ...
    function init() {
        ...
        configuration.on("change:param", function(context, mode) {
            ...
            d3.selectAll(".air_quality-mode").classed("invisible", mode !== "air_quality");
            switch (mode) {
                case "wind":
        ...
        d3.select("#air_quality-mode-enable").on("click", function() {
            if (configuration.get("param") !== "air_quality") {
                configuration.save({param: "air_quality", surface: "surface", level: "level", overlayType: "pm2_5"});
            }
        });
        configuration.on("change:param", function(x, param) {
            d3.select("#air_quality-mode-enable").classed("highlighted", param === "air_quality");
        });
        ...
        bindButtonToConfiguration("#overlay-air_quality-off", {overlayType: "off"});
